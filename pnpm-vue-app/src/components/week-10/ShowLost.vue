...
const handlePageChange = async (page) => {
  ...
  // 根据新的页码值获取对应页的数据
  try {
    const response = await axios.get(`http://localhost:8080/your-api-endpoint?page=${page}`);
    // 处理获取到的数据，例如更新 cardData
...
const getData = () => {
  axios.get('http://localhost:8080/lost', {
    params: {
...
// ])
const aa = 'http://localhost:8080/image/'
...
const filterItems = () => {
  ...
  axios.post('http://localhost:8080/lost/find', filters)
      .then(response => {
        data.value = response.data;
        // 处理返回的符合条件的失物数据
      })
