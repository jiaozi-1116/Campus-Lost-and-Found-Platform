...
const getData = () => {
  axios.get('http://localhost:8080/lost/findByPage', {
    params: {
...
const handleSearchName = (val) => {
  if(val.length>0){
    axios.get(`http://localhost:8080/lostByName/${val}`).then(response=>{
...
const dialogOk = () => {
  ...
  if (dialogType.value === 'add') {
    ...
    console.log(newUser)
    axios.post('http://localhost:8080/lost', newUser)
    ...
    axios.put(`http://localhost:8080/lost/${userId}`, updatedUser)
        .then(response => {
          // 处理成功响应
...
const delrow = (row) => {
  const userId = row.id;
  axios.delete(`http://localhost:8080/lost/${userId}`)
      .then(response => {
        // 处理删除成功后的逻辑，例如重新加载数据
        getData();
      })
